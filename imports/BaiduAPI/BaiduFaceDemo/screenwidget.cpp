...
ScreenWidget::ScreenWidget(QWidget *parent) : QWidget(parent)
{
    ...
    menu = new QMenu(this);
    menu->addAction(QIcon(":/icons/White/069-ok.png"),"完成", this, SLOT(saveScreen()));
//    menu->addAction("保存全屏截图", this, SLOT(saveFullScreen()));
    menu->addAction(QIcon(":/icons/White/064-save.png"),"保存当前截图", this, SLOT(saveScreenOther()));
    menu->addAction(QIcon(":/icons/White/064-save.png"),"保存全屏截图", this, SLOT(saveFullOther()));
    menu->addAction(QIcon(":/icons/White/068-cancel.png"),"退出", this, SLOT(close()));
...
void ScreenWidget::showEvent(QShowEvent *)
{
    ...
    *fullScreen = pscreen->grabWindow(QApplication::desktop()->winId(), 0, 0, screen->width(), screen->height());
    ...
    pix.fill((QColor(80, 160, 160, 128)));
...
void ScreenWidget::saveScreen()
{
    ...
    int w = screen->getRightDown().x() - x;
    int h = screen->getRightDown().y() - y;
    emit overShot(fullScreen->copy(x, y, w, h));
...
void ScreenWidget::keyPressEvent(QKeyEvent *e_key)
{
    if(e_key->key() == Qt::Key_Enter || e_key->key() == Qt::Key_Return)
    {
        int x = screen->getLeftUp().x();
        int y = screen->getLeftUp().y();
        int w = screen->getRightDown().x() - x;
    ...
    else if(e_key->key() == Qt::Key_Escape)
    {
        close();
    }
}
