...
//人脸检测
void BaiduFaceOnline::FaceDetect(const QImage& image)
{
    ...
    connect(NetAccManager,&QNetworkAccessManager::finished,this,&BaiduFaceOnline::FaceDetectReply);
    NetAccManager->post(NetRequest,post_param);
}
//人脸检测槽
void BaiduFaceOnline::FaceDetectReply(QNetworkReply* reply)
{
    QString error = reply->errorString();
    if (!error.isEmpty() && error != "Unknown error")
    {
    ...
    disconnect(NetAccManager,&QNetworkAccessManager::finished,this,&BaiduFaceOnline::FaceDetectReply);
}
//人脸对比
...
//人脸注册
void BaiduFaceOnline::FaceRegister(const QImage& image, const QString& group_id, const QString& user_id)
{
    QString img_base64 = image2base64_str(image);
    //打包请求参数
    QJsonObject post_data;;
    ...
    NetRequest.setUrl(url);
    NetRequest.setHeader(QNetworkRequest::ContentTypeHeader, QVariant("application/json")); //接口固定
    connect(NetAccManager,&QNetworkAccessManager::finished,this,&BaiduFaceOnline::FaceRegisterReply);
    NetAccManager->post(NetRequest,post_param);
}
//人脸注册槽
void BaiduFaceOnline::FaceRegisterReply(QNetworkReply* reply)
{
    if (reply->error() != QNetworkReply::NoError)
    {
        QMessageBox::warning(0,"","请求错误！");
    ...
    disconnect(NetAccManager,&QNetworkAccessManager::finished,this,&BaiduFaceOnline::FaceRegisterReply);
}
//人脸搜索识别
void BaiduFaceOnline::FaceSerch(const QImage& image)
{
    QString img_base64 = image2base64_str(image);
    //打包请求参数
    QJsonObject post_data;;
    ...
    NetRequest.setUrl(url);
    NetRequest.setHeader(QNetworkRequest::ContentTypeHeader, QVariant("application/json")); //接口固定
    connect(NetAccManager,&QNetworkAccessManager::finished,this,&BaiduFaceOnline::FaceSerchReply);
    NetAccManager->post(NetRequest,post_param);
}
//人脸搜索识别槽
void BaiduFaceOnline::FaceSerchReply(QNetworkReply* reply)
{
    if (reply->error() != QNetworkReply::NoError)
    {
        QMessageBox::warning(0,"","请求错误！");
    ...
    disconnect(NetAccManager,&QNetworkAccessManager::finished,this,&BaiduFaceOnline::FaceSerchReply);
}
//证件识别
void BaiduFaceOnline::CardScan(const QImage& image)
{
    QString img_base64 = image2base64_str(image);
}
//证件识别槽
void BaiduFaceOnline::CardScanReply(QNetworkReply* reply)
{
